...
typedef Noise_KK_device_t        device;
typedef Noise_KK_session_t       session;
typedef Noise_KK_peer_t          peer;
typedef Noise_KK_encap_message_t encap_message;
typedef Noise_KK_rcode           rcode;
...
int main () {
    ...
    Noise_KK_dh_secret_to_public(alice_spub, alice_spriv);
    Noise_KK_dh_secret_to_public(bob_spub, bob_spriv);

    /*
     * Initialize Alice's device
     */
    // Create the device
    device *alice_device = Noise_KK_device_create(10, prologue, (uint8_t*) "Alice",
                                               alice_srlz_key, alice_spriv);
    
    // Register Bob
    peer *peer_bob = Noise_KK_device_add_peer(alice_device, (uint8_t*) "Bob", bob_spub);
    if (!peer_bob) return 1;
    peer_id bob_id = Noise_KK_peer_get_id(peer_bob);

    /*
     * Initialize Bob's device
     */
    // Create the device
    device *bob_device = Noise_KK_device_create(10, prologue, (uint8_t*) "Bob",
                                             bob_srlz_key, bob_spriv);

    // Register Alice
    peer *peer_alice = Noise_KK_device_add_peer(bob_device, (uint8_t*) "Alice", alice_spub);
    if (!peer_alice) return 1;
    peer_id alice_id = Noise_KK_peer_get_id(peer_alice);
    ...
    session *alice_session = Noise_KK_session_create_initiator(alice_device, bob_id);
    RETURN_IF_ERROR(alice_session, "Alice session creation");

    // Bob, however, uses the first message to learn Alice's identity.
    session *bob_session = Noise_KK_session_create_responder(bob_device, alice_id);
    ...
    encap_msg = Noise_KK_pack_message_with_conf_level(NOISE_KK_CONF_ZERO, 0, NULL);
    res = Noise_KK_session_write(encap_msg, alice_session, &cipher_msg_len, &cipher_msg);
    RETURN_IF_ERROR(Noise_KK_rcode_is_success(res), "Send message 0");
    Noise_KK_encap_message_p_free(encap_msg);

    // ## Bob: read the message
    res = Noise_KK_session_read(&encap_msg, bob_session, cipher_msg_len, cipher_msg);
    RETURN_IF_ERROR(Noise_KK_rcode_is_success(res), "Receive message 0");
    ...
    RETURN_IF_ERROR(
                    Noise_KK_unpack_message_with_auth_level(&plain_msg_len, &plain_msg,
                                                         NOISE_KK_AUTH_ZERO, encap_msg),
                    "Unpack message 0");
    Noise_KK_encap_message_p_free(encap_msg);
    ...
    encap_msg = Noise_KK_pack_message_with_conf_level(NOISE_KK_CONF_ZERO, 0, NULL);
    res = Noise_KK_session_write(encap_msg, bob_session, &cipher_msg_len, &cipher_msg);
    RETURN_IF_ERROR(Noise_KK_rcode_is_success(res), "Send message 1");
    Noise_KK_encap_message_p_free(encap_msg);

    // ## Alice: read the message
    res = Noise_KK_session_read(&encap_msg, alice_session, cipher_msg_len, cipher_msg);
    RETURN_IF_ERROR(Noise_KK_rcode_is_success(res), "Receive message 1");
    RETURN_IF_ERROR(
                    Noise_KK_unpack_message_with_auth_level(&plain_msg_len, &plain_msg,
                                                         NOISE_KK_AUTH_ZERO, encap_msg),
                    "Unpack message 1");
    ...
    encap_msg = Noise_KK_pack_message_with_conf_level(NOISE_KK_CONF_STRONG_FORWARD_SECRECY,
        11, (uint8_t*) "Hello Bob!");
    res = Noise_KK_session_write(encap_msg, alice_session, &cipher_msg_len, &cipher_msg);
    RETURN_IF_ERROR(Noise_KK_rcode_is_success(res), "Send message 2");
    Noise_KK_encap_message_p_free(encap_msg);

    // ## Bob: read the message
    // We request the highest authentication guarantees (known sender, no KCI).
    res = Noise_KK_session_read(&encap_msg, bob_session, cipher_msg_len, cipher_msg);
    RETURN_IF_ERROR(Noise_KK_rcode_is_success(res), "Receive message 2");
    RETURN_IF_ERROR(
                    Noise_KK_unpack_message_with_auth_level(&plain_msg_len, &plain_msg,
                                                         NOISE_KK_AUTH_KNOWN_SENDER_NO_KCI,
                                                         encap_msg),
                    "Unpack message 2");
    Noise_KK_encap_message_p_free(encap_msg);
    if (cipher_msg_len > 0) free(cipher_msg);
    ...
    res = Noise_KK_session_write(encap_msg, bob_session, &cipher_msg_len, &cipher_msg);
    RETURN_IF_ERROR(Noise_KK_rcode_is_success(res), "Send message 3");
    Noise_KK_encap_message_p_free(encap_msg);

   // ## Alice: read the message (request max authentication)
    res = Noise_KK_session_read(&encap_msg, alice_session, cipher_msg_len, cipher_msg);
    RETURN_IF_ERROR(Noise_KK_rcode_is_success(res), "Receive message 3");
    RETURN_IF_ERROR(
                    Noise_KK_unpack_message_with_auth_level(&plain_msg_len, &plain_msg,
                                                         NOISE_KK_AUTH_KNOWN_SENDER_NO_KCI,
                                                         encap_msg),
    ...
    Noise_KK_session_free(alice_session);
    Noise_KK_session_free(bob_session);
    Noise_KK_device_free(alice_device);
    Noise_KK_device_free(bob_device);
