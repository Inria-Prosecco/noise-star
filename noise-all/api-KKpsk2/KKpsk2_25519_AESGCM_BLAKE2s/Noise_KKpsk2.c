...
inline bool Noise_KKpsk2_lbytes_eq(uint32_t len, uint8_t *b1, uint8_t *b2)
{
...
)
{
  ...
  switch (r)
  {
    ...
    case EverCrypt_Error_AuthenticationFailure:
      {
        return Noise_KKpsk2_CDecrypt_error;
      }
    default:
      {
...
)
{
  ...
  Noise_KKpsk2_hmac(secret, (uint32_t)32U, hash1, keylen, key);
  if (!(dst1 == NULL))
  {
    output[0U] = (uint8_t)1U;
    Noise_KKpsk2_hmac(output_hash, (uint32_t)32U, secret, (uint32_t)1U, output1);
    memcpy(dst1, output_hash, (uint32_t)32U * sizeof (uint8_t));
    if (!(dst2 == NULL))
    {
      output[32U] = (uint8_t)2U;
      Noise_KKpsk2_hmac(output_hash, (uint32_t)32U, secret, (uint32_t)33U, output);
...
void Noise_KKpsk2_mix_psk(uint8_t *psk, uint8_t *st_cs_k, uint8_t *st_ck, uint8_t *st_h)
{
  ...
  Noise_KKpsk2_kdf(st_ck, (uint32_t)32U, psk, st_ck, temp_hash, st_cs_k);
  Noise_KKpsk2_mix_hash(st_h, (uint32_t)32U, temp_hash);
...
{
  Noise_KKpsk2_aead_encrypt(st_cs_k, nonce, (uint32_t)32U, st_h, msg_len, msg, cipher);
  uint32_t cipher_len = msg_len + (uint32_t)16U;
  Noise_KKpsk2_mix_hash(st_h, cipher_len, cipher);
}
...
{
  Noise_KKpsk2_error_code
  r1 = Noise_KKpsk2_aead_decrypt(st_cs_k, nonce, (uint32_t)32U, st_h, msg_len, msg, cipher);
  if (r1 == Noise_KKpsk2_CSuccess)
  {
    Noise_KKpsk2_mix_hash(st_h, msg_len + (uint32_t)16U, cipher);
    return Noise_KKpsk2_CSuccess;
  }
  else
    return r1;
...
)
{
  ...
  Noise_KKpsk2_error_code r1 = Noise_KKpsk2_dh(dh_key, sec, pub);
  Noise_KKpsk2_error_code r2;
  if (r1 == Noise_KKpsk2_CSuccess)
  {
    Noise_KKpsk2_kdf(ck, (uint32_t)32U, dh_key, ck, cipher_key, NULL);
    r2 = Noise_KKpsk2_CSuccess;
  }
  else
    r2 = r1;
  Noise_KKpsk2_error_code r = r2;
