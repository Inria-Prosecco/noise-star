...
)
{
  ...
  Noise_Kpsk0_hmac(secret, (uint32_t)64U, hash1, keylen, key);
  if (!(dst1 == NULL))
  {
    output[0U] = (uint8_t)1U;
    Noise_Kpsk0_hmac(output_hash, (uint32_t)64U, secret, (uint32_t)1U, output1);
    memcpy(dst1, output_hash, (uint32_t)64U * sizeof (uint8_t));
    if (!(dst2 == NULL))
    {
      output[64U] = (uint8_t)2U;
      Noise_Kpsk0_hmac(output_hash, (uint32_t)64U, secret, (uint32_t)65U, output);
...
void Noise_Kpsk0_mix_psk(uint8_t *psk, uint8_t *st_cs_k, uint8_t *st_ck, uint8_t *st_h)
{
  ...
  Noise_Kpsk0_kdf(st_ck, (uint32_t)32U, psk, st_ck, temp_hash, temp_k);
  memcpy(st_cs_k, temp_k, (uint32_t)32U * sizeof (uint8_t));
  Lib_Memzero0_memzero(temp_k, (uint32_t)64U * sizeof (temp_k[0U]));
  Noise_Kpsk0_mix_hash(st_h, (uint32_t)64U, temp_hash);
...
{
  Noise_Kpsk0_aead_encrypt(st_cs_k, nonce, (uint32_t)64U, st_h, msg_len, msg, cipher);
...
{
  Noise_Kpsk0_error_code
  r1 = Noise_Kpsk0_aead_decrypt(st_cs_k, nonce, (uint32_t)64U, st_h, msg_len, msg, cipher);
...
)
{
  ...
  {
    uint8_t temp_k[64U] = { 0U };
    Noise_Kpsk0_kdf(ck, (uint32_t)32U, dh_key, ck, temp_k, NULL);
