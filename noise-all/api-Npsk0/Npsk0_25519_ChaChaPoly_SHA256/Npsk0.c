...
Prims_int Noise_num_pattern_messages = (krml_checked_int_t)1;
          ...
          if (res0 == Noise_CSuccess)
            ...
            switch (res0)
            {
              case Noise_CDH_error:
                {
                  res =
                    ((result_init_state_t){ .tag = Fail, .val = { .case_Fail = Noise_CDH_error } });
                  break;
                }
              default:
                {
...
)
{
  ...
  else if (sn.tag == Noise_DS_Responder)
  {
    ...
    if (sn_state.tag == Noise_IMS_Transport)
    {
      ...
      out[0U] = NULL;
      return
        ((Noise_rcode){ .tag = Noise_Error, .val = { .case_Error = Noise_CIncorrect_transition } });
    }
    else if (sn_state.tag == Noise_IMS_Handshake)
    {
      uint32_t st_step = sn_state.val.case_IMS_Handshake.step;
...
*/
Noise_session_t
*Noise__session_create_responder_with_ephemeral(Noise_device_t *dvp, uint32_t pid)
{
  Noise_device_t dv = dvp[0U];
  result_session_t res0;
  if (dv.dv_states_counter == (uint32_t)4294967295U)
