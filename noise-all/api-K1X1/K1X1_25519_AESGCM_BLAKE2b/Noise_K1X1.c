...
)
{
  ...
  Noise_K1X1_hmac(secret, (uint32_t)64U, hash1, keylen, key);
  if (!(dst1 == NULL))
  {
    output[0U] = (uint8_t)1U;
    Noise_K1X1_hmac(output_hash, (uint32_t)64U, secret, (uint32_t)1U, output1);
    memcpy(dst1, output_hash, (uint32_t)64U * sizeof (uint8_t));
    if (!(dst2 == NULL))
    {
      output[64U] = (uint8_t)2U;
      Noise_K1X1_hmac(output_hash, (uint32_t)64U, secret, (uint32_t)65U, output);
...
void Noise_K1X1_mix_psk(uint8_t *psk, uint8_t *st_cs_k, uint8_t *st_ck, uint8_t *st_h)
{
  ...
  Noise_K1X1_kdf(st_ck, (uint32_t)32U, psk, st_ck, temp_hash, temp_k);
  memcpy(st_cs_k, temp_k, (uint32_t)32U * sizeof (uint8_t));
  Lib_Memzero0_memzero(temp_k, (uint32_t)64U * sizeof (temp_k[0U]));
  Noise_K1X1_mix_hash(st_h, (uint32_t)64U, temp_hash);
...
{
  Noise_K1X1_aead_encrypt(st_cs_k, nonce, (uint32_t)64U, st_h, msg_len, msg, cipher);
...
{
  Noise_K1X1_error_code
  r1 = Noise_K1X1_aead_decrypt(st_cs_k, nonce, (uint32_t)64U, st_h, msg_len, msg, cipher);
...
Noise_K1X1_mix_dh(uint8_t *sec, uint8_t *pub, uint8_t *cipher_key, uint8_t *ck, uint8_t *hash1)
{
  ...
  {
    uint8_t temp_k[64U] = { 0U };
    Noise_K1X1_kdf(ck, (uint32_t)32U, dh_key, ck, temp_k, NULL);
