...
Prims_int Noise_num_pattern_messages = (krml_checked_int_t)3;
          ...
          if (res0 == Noise_CSuccess)
            ...
            switch (res0)
            {
              case Noise_CDH_error:
                {
                  res =
                    ((result_init_state_t){ .tag = Fail0, .val = { .case_Fail = Noise_CDH_error } });
                  break;
                }
              default:
                {
    ...
    if (dst_st.tag == Noise_IMS_Handshake)
    {
      ...
      else
      {
        ...
        if (!(payload_outlen <= (uint32_t)4294967199U && inlen == (uint32_t)96U + payload_outlen))
          r = ((result_unit_error){ .tag = Fail0, .v = Noise_CInput_size });
        else
        {
          uint8_t *msg1 = input;
          ...
          if (r11 == Noise_CSuccess)
          {
            ...
            Noise_error_code
            r = Noise_decrypt_and_hash((uint32_t)32U, st_rs, input_1, st_cipher, st_h, (uint64_t)0U);
          ...
          if (r1 == Noise_CSuccess)
          {
            ...
            if (r0)
            {
              ...
              r = ((result_unit_error){ .tag = Fail0, .v = Noise_CRs_rejected_by_policy });
          }
          else
