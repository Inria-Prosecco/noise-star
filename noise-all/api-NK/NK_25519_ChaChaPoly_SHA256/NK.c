...
typedef Noise_NK_cell **t___Impl_Noise_API_Instances_KK_25519_ChaChaPoly_SHA256_peer_p;
...
{
  Noise_NK_init_state_t_tags tag;
  union {
    struct 
    {
      uint32_t step;
...
void Noise_NK_session_free(Noise_NK_session_t *sn)
{
  ...
  if (st.tag == Noise_NK_DS_Initiator)
  {
    ...
    Noise_NK_init_state_t state = st.val.case_DS_Initiator.state;
  ...
  else if (st.tag == Noise_NK_DS_Responder)
  {
    ...
    Noise_NK_resp_state_t state = st.val.case_DS_Responder.state;
    if (state.tag == Noise_NK_IMS_Handshake)
    {
      uint8_t *st_re = state.val.case_IMS_Handshake.re;
      uint8_t *st_epub = state.val.case_IMS_Handshake.epub;
...
)
{
  ...
  if (dst.tag == Noise_NK_DS_Initiator)
  {
    ...
    Noise_NK_init_state_t dst_st = dst.val.case_DS_Initiator.state;
    if (dst_st.tag == Noise_NK_IMS_Handshake)
    {
      ...
      else
      {
        ...
        if (!(payload_len <= (uint32_t)4294967247U && outlen == (uint32_t)48U + payload_len))
          res = ((result_init_state_t){ .tag = Fail, .val = { .case_Fail = Noise_NK_CInput_size } });
        else
        {
  ...
  else if (dst.tag == Noise_NK_DS_Responder)
  {
    ...
    Noise_NK_resp_state_t dst_st = dst.val.case_DS_Responder.state;
    if (dst_st.tag == Noise_NK_IMS_Handshake)
    {
      uint8_t *st_re = dst_st.val.case_IMS_Handshake.re;
      uint8_t *st_epub = dst_st.val.case_IMS_Handshake.epub;
      ...
      else
      {
        ...
        if (!(payload_len <= (uint32_t)4294967247U && outlen == (uint32_t)48U + payload_len))
          res = ((result_resp_state_t){ .tag = Fail, .val = { .case_Fail = Noise_NK_CInput_size } });
        else
        {
          uint32_t pat_outlen = (uint32_t)32U;
          uint8_t *pat_out = out;
  ...
  if (res0.tag == Fail)
  {
    ...
    if (dst1.tag == Noise_NK_DS_Initiator)
    {
      ...
      Noise_NK_init_state_t st = dst1.val.case_DS_Initiator.state;
    ...
    else if (dst1.tag == Noise_NK_DS_Responder)
    {
      ...
      Noise_NK_resp_state_t st = dst1.val.case_DS_Responder.state;
...
)
{
  ...
  if (dst.tag == Noise_NK_DS_Initiator)
  {
    ...
    Noise_NK_init_state_t state = dst.val.case_DS_Initiator.state;
    bool ite;
    if (state.tag == Noise_NK_IMS_Handshake)
      ite = true;
    else
    ...
    if (!ite)
    {
      ...
      if (state.tag == Noise_NK_IMS_Transport)
      {
        ...
        if (!(plen <= (uint32_t)4294967279U && clen == plen + (uint32_t)16U))
          scrut =
            ((result_init_state_t){ .tag = Fail, .val = { .case_Fail = Noise_NK_CInput_size } });
  ...
  else if (dst.tag == Noise_NK_DS_Responder)
  {
    ...
    Noise_NK_resp_state_t state = dst.val.case_DS_Responder.state;
    bool ite;
    if (state.tag == Noise_NK_IMS_Handshake)
      ite = true;
    else
    ...
    if (!ite)
    {
      ...
      if (state.tag == Noise_NK_IMS_Transport)
      {
        ...
        if (!(plen <= (uint32_t)4294967279U && clen == plen + (uint32_t)16U))
          scrut =
            ((result_resp_state_t){ .tag = Fail, .val = { .case_Fail = Noise_NK_CInput_size } });
        else if (send_nonce >= (uint64_t)18446744073709551615U)
          scrut =
            (
              (result_resp_state_t){
...
)
{
  ...
  {
    Noise_NK_init_state_t sn_state = sn.val.case_DS_Initiator.state;
  ...
  {
    Noise_NK_resp_state_t sn_state = sn.val.case_DS_Responder.state;
    if (sn_state.tag == Noise_NK_IMS_Transport)
    {
...
)
{
  ...
  if (dst.tag == Noise_NK_DS_Initiator)
  {
    Noise_NK_init_state_t st = dst.val.case_DS_Initiator.state;
    if (st.tag == Noise_NK_IMS_Handshake)
    {
  ...
  {
    Noise_NK_resp_state_t st = dst.val.case_DS_Responder.state;
    if (st.tag == Noise_NK_IMS_Handshake)
    {
      uint32_t step = st.val.case_IMS_Handshake.step;
      option__uint32_t scrut;
