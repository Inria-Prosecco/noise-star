...
bool Noise_XX_uu___is_Success(Noise_XX_rcode projectee)
{
  if (projectee.tag == Noise_XX_Success)
    return true;
  else
    return false;
}

bool Noise_XX_uu___is_Error(Noise_XX_rcode projectee)
{
  if (projectee.tag == Noise_XX_Error)
    return true;
  else
    return false;
}

Noise_XX_error_code Noise_XX___proj__Error__item___0(Noise_XX_rcode projectee)
{
  if (projectee.tag == Noise_XX_Error)
...
bool Noise_XX_uu___is_Stuck(Noise_XX_rcode projectee)
{
  if (projectee.tag == Noise_XX_Stuck)
    return true;
  else
    return false;
}

Noise_XX_error_code Noise_XX___proj__Stuck__item___0(Noise_XX_rcode projectee)
{
  if (projectee.tag == Noise_XX_Stuck)
...
bool Noise_XX_uu___is_Auth_level(Noise_XX_ac_level_t projectee)
{
  if (projectee.tag == Noise_XX_Auth_level)
    return true;
  else
    return false;
}

uint8_t Noise_XX___proj__Auth_level__item__l(Noise_XX_ac_level_t projectee)
{
  if (projectee.tag == Noise_XX_Auth_level)
...
bool Noise_XX_uu___is_Conf_level(Noise_XX_ac_level_t projectee)
{
  if (projectee.tag == Noise_XX_Conf_level)
    return true;
  else
    return false;
}

uint8_t Noise_XX___proj__Conf_level__item__l(Noise_XX_ac_level_t projectee)
{
  if (projectee.tag == Noise_XX_Conf_level)
...
bool Noise_XX_uu___is_No_level(Noise_XX_ac_level_t projectee)
{
  if (projectee.tag == Noise_XX_No_level)
    return true;
  else
    return false;
}

typedef struct Noise_XX_encap_message_t_s
{
  Noise_XX_ac_level_t em_ac_level;
  uint32_t em_message_len;
  uint8_t *em_message;
}
Noise_XX_encap_message_t;

Noise_XX_encap_message_t
*Noise_XX___proj__Mkencap_message_p_or_null__item__emp(Noise_XX_encap_message_t *projectee)
{
  return projectee;
}

bool Noise_XX_encap_message_p_is_null(Noise_XX_encap_message_t *emp)
{
  return emp == NULL;
}

void Noise_XX_encap_message_p_free(Noise_XX_encap_message_t *emp)
{
  Noise_XX_encap_message_t em = emp[0U];
...
Noise_XX_encap_message_t
*Noise_XX_pack_message_with_conf_level(
...
)
{
  ...
  if (msg_len > (uint32_t)0U)
  {
    ...
    msg_ = NULL;
  KRML_CHECK_SIZE(sizeof (Noise_XX_encap_message_t), (uint32_t)1U);
  Noise_XX_encap_message_t *emp_p = KRML_HOST_MALLOC(sizeof (Noise_XX_encap_message_t));
  emp_p[0U]
  =
    (
      (Noise_XX_encap_message_t){
        .em_ac_level = {
          .tag = Noise_XX_Conf_level,
...
Noise_XX_encap_message_t *Noise_XX_pack_message(uint32_t msg_len, uint8_t *msg)
{
  return Noise_XX_pack_message_with_conf_level(NOISE_XX_MAX_CONF_LEVEL, msg_len, msg);
}

bool
Noise_XX_unpack_message_with_auth_level(
  uint32_t *out_msg_len,
  uint8_t **out_msg,
  uint8_t requested_auth_level,
  Noise_XX_encap_message_t *emp
)
{
  Noise_XX_encap_message_t em = emp[0U];
  bool ok;
  if (em.em_message_len == (uint32_t)0U)
    ok = true;
  else if (em.em_ac_level.tag == Noise_XX_Auth_level)
  {
...
{
  return
    Noise_XX_unpack_message_with_auth_level(out_msg_len,
      out_msg,
      NOISE_XX_MAX_AUTH_LEVEL,
      emp);
}

void
Noise_XX_unsafe_unpack_message(
  Noise_XX_ac_level_t *out_ac_level,
  uint32_t *out_msg_len,
  uint8_t **out_msg,
  Noise_XX_encap_message_t *emp
)
...
Prims_int Noise_XX_num_pattern_messages = (krml_checked_int_t)3;

bool Noise_XX_rcode_is_success(Noise_XX_rcode c)
{
  if (c.tag == Noise_XX_Success)
    return true;
  else
    return false;
}

bool Noise_XX_rcode_is_error(Noise_XX_rcode c)
{
  if (c.tag == Noise_XX_Error)
    return true;
  else
    return false;
}

bool Noise_XX_rcode_is_stuck(Noise_XX_rcode c)
{
  if (c.tag == Noise_XX_Stuck)
    return true;
  else
    return false;
}

typedef struct Noise_XX_init_state_t_s
{
  Noise_XX_init_state_t_tags tag;
  union {
    struct 
    {
      uint32_t step;
...
Noise_XX_init_state_t;

typedef struct Noise_XX_peer_t_s
{
  uint32_t p_id;
  Noise_XX_noise_string *p_info;
  uint8_t *p_s;
}
Noise_XX_peer_t;

typedef Noise_XX_peer_t *peer_p;

typedef Noise_XX_cell **t___Impl_Noise_API_Instances_X1N_25519_AESGCM_BLAKE2b_peer_p;

typedef struct Noise_XX_device_t_s
{
  Noise_XX_noise_string *dv_info;
  uint8_t *dv_sk;
  uint8_t *dv_spriv;
  uint8_t *dv_spub;
  Noise_XX_sized_buffer dv_prologue;
  uint32_t dv_states_counter;
  Noise_XX_cell **dv_peers;
  uint32_t dv_peers_counter;
}
Noise_XX_device_t;

typedef Noise_XX_device_t *device_p;

typedef struct Noise_XX_resp_state_t_s
{
  Noise_XX_init_state_t_tags tag;
  union {
    struct 
...
Noise_XX_resp_state_t;

typedef struct Noise_XX_session_t_s
{
  Noise_XX_session_t_tags tag;
  union {
    struct 
    {
      Noise_XX_init_state_t state;
      uint32_t id;
      Noise_XX_noise_string *info;
      uint8_t *spriv;
      uint8_t *spub;
      uint32_t pid;
      Noise_XX_noise_string *pinfo;
      Noise_XX_device_t *dv;
    }
    case_DS_Initiator;
    struct 
    {
      Noise_XX_resp_state_t state;
      uint32_t id;
      Noise_XX_noise_string *info;
      uint8_t *spriv;
      uint8_t *spub;
...
typedef Noise_XX_session_t *session_p;
...
)
{
  ...
  Noise_XX_error_code res = Noise_XX_dh_secret_to_public(spub_, spriv_);
  switch (res)
  {
    case Noise_XX_CSuccess:
      {
        uint8_t *prlg_;
        if (prlg_len > (uint32_t)0U)
        {
          ...
          prlg_ = NULL;
        Noise_XX_sized_buffer prlg_1 = { .size = prlg_len, .buffer = prlg_ };
        bool b = info == NULL;
        uint8_t *out_str;
        if (b)
          out_str = NULL;
        ...
        out_ptr[0U] = out_str;
        Noise_XX_noise_string *info_ = out_ptr;
        KRML_CHECK_SIZE(sizeof (Noise_XX_cell *), (uint32_t)1U);
        Noise_XX_cell **ptr = KRML_HOST_MALLOC(sizeof (Noise_XX_cell *));
        ptr[0U] = NULL;
        Noise_XX_cell **peers = ptr;
        Noise_XX_device_t
        dv =
          {
        ...
        KRML_CHECK_SIZE(sizeof (Noise_XX_device_t), (uint32_t)1U);
        Noise_XX_device_t *dvp = KRML_HOST_MALLOC(sizeof (Noise_XX_device_t));
        dvp[0U] = dv;
        return dvp;
      }
    default:
...
*/
Noise_XX_device_t
*Noise_XX_device_create_from_secret(
  uint32_t prlg_len,
  uint8_t *prlg,
  uint8_t *info,
  uint8_t *sk,
...
)
{
  ...
  uint64_t n0 = Noise_XX_bytes_to_nonce(n8);
  Noise_XX_error_code
  res0 = Noise_XX_aead_decrypt(sk, n0, name_raw_len, name_raw, (uint32_t)32U, spriv_, c0);
  if (!(res0 == Noise_XX_CSuccess))
  {
  ...
  else
  {
    ...
    uint8_t *sk_ = o0;
    uint8_t *spub_ = KRML_HOST_CALLOC((uint32_t)32U, sizeof (uint8_t));
    Noise_XX_error_code res1 = Noise_XX_dh_secret_to_public(spub_, spriv_);
    uint8_t *prlg_;
    if (prlg_len > (uint32_t)0U)
    {
      KRML_CHECK_SIZE(sizeof (uint8_t), prlg_len);
...
/*
  ...
  from this device.
*/
void Noise_XX_device_free(Noise_XX_device_t *dvp)
{
  Noise_XX_device_t dv = dvp[0U];
...
*/
void Noise_XX_serialize_device_secret(uint32_t *outlen, uint8_t **out, Noise_XX_device_t *dvp)
{
  Noise_XX_device_t dv = dvp[0U];
  ...
  uint64_t n = Noise_XX_bytes_to_nonce(n8);
  Noise_XX_aead_encrypt(dv.dv_sk, n, name_raw_len, name_raw, (uint32_t)32U, dv.dv_spriv, c);
  out[0U] = outb;
...
{
  Noise_XX_cell *l = *pl;
  Noise_XX_cell c = { .next = l, .data = x };
  KRML_CHECK_SIZE(sizeof (Noise_XX_cell), (uint32_t)1U);
  Noise_XX_cell *pc = KRML_HOST_MALLOC(sizeof (Noise_XX_cell));
  pc[0U] = c;
  *pl = pc;
}
...
*/
Noise_XX_peer_t *Noise_XX_device_add_peer(Noise_XX_device_t *dvp, uint8_t *pinfo, uint8_t *rs)
{
  Noise_XX_device_t dv = dvp[0U];
  uint32_t pcounter = dv.dv_peers_counter;
  bool b1 = pcounter == (uint32_t)4294967295U;
  Noise_XX_cell *llt = *dv.dv_peers;
  Noise_XX_cell *lltp = llt;
  Noise_XX_cell *llt10 = lltp;
  bool b0;
  if (llt10 == NULL)
    b0 = false;
  else
  {
    Noise_XX_cell c = llt10[0U];
    Noise_XX_peer_t x = c.data[0U];
    bool b = Noise_XX_lbytes_eq((uint32_t)32U, x.p_s, rs);
    bool b1 = b;
    b0 = !b1;
  }
  bool cond = b0;
  while (cond)
  {
    Noise_XX_cell *llt1 = lltp;
    Noise_XX_cell c0 = llt1[0U];
    lltp = c0.next;
    Noise_XX_cell *llt10 = lltp;
    bool b;
    if (llt10 == NULL)
    ...
    else
    {
      Noise_XX_cell c = llt10[0U];
      Noise_XX_peer_t x = c.data[0U];
      bool b0 = Noise_XX_lbytes_eq((uint32_t)32U, x.p_s, rs);
      bool b1 = b0;
      b = !b1;
    }
    cond = b;
  ...
  {
    Noise_XX_noise_string *info1 = dv.dv_info;
    uint8_t *sk1 = dv.dv_sk;
    uint8_t *spriv1 = dv.dv_spriv;
    uint8_t *spub1 = dv.dv_spub;
    Noise_XX_sized_buffer prologue1 = dv.dv_prologue;
    uint32_t scounter1 = dv.dv_states_counter;
    Noise_XX_cell **peers1 = dv.dv_peers;
    ...
    uint8_t **out_ptr = KRML_HOST_MALLOC(sizeof (uint8_t *));
    out_ptr[0U] = out_str;
    Noise_XX_noise_string *pinfo1 = out_ptr;
    Noise_XX_peer_t x_ = { .p_id = pcounter1, .p_info = pinfo1, .p_s = rs1 };
    KRML_CHECK_SIZE(sizeof (Noise_XX_peer_t), (uint32_t)1U);
    Noise_XX_peer_t *xp_ = KRML_HOST_MALLOC(sizeof (Noise_XX_peer_t));
    xp_[0U] = x_;
    Noise_XX_peer_t *x = xp_;
    ...
    Noise_XX_peer_t *pp0 = pp;
    return pp0;
  }
}

static Noise_XX_peer_t
...
{
  Noise_XX_cell *l = *pl;
  Noise_XX_peer_t *r1 = (*l).data;
  Noise_XX_cell *next = (*l).next;
  *pl = next;
  KRML_HOST_FREE(l);
  return r1;
}
...
*/
void Noise_XX_device_remove_peer(Noise_XX_device_t *dvp, uint32_t pid)
{
  if (!(pid == (uint32_t)0U))
  {
    Noise_XX_device_t dv = dvp[0U];
    ...
    if (!(llt == NULL))
    {
      Noise_XX_cell c0 = *llt;
      Noise_XX_peer_t x = c0.data[0U];
      if (x.p_id != pid)
      {
        Noise_XX_cell *llt1 = *dv.dv_peers;
        Noise_XX_cell *lltp = llt1;
        Noise_XX_cell *llt20 = lltp;
        Noise_XX_cell *next = llt20->next;
        bool b0;
        if (next == NULL)
          b0 = false;
        else
        {
          Noise_XX_cell c = next[0U];
          Noise_XX_peer_t x = c.data[0U];
          b0 = x.p_id != pid;
        }
        bool cond = b0;
        while (cond)
        {
          Noise_XX_cell *llt2 = lltp;
          Noise_XX_cell c0 = llt2[0U];
          lltp = c0.next;
          Noise_XX_cell *llt20 = lltp;
          Noise_XX_cell *next = llt20->next;
          bool b;
        ...
        Noise_XX_cell *llt2 = *&lltp;
        Noise_XX_cell c01 = *llt2;
        if (!(c01.next == NULL))
        {
          Noise_XX_cell c1 = *c01.next;
          llt2[0U] = ((Noise_XX_cell){ .next = c1.next, .data = c01.data });
          Noise_XX_peer_t p = c1.data[0U];
...
*/
void
Noise_XX_serialize_peer_secret(
  uint32_t *outlen,
  uint8_t **out,
  Noise_XX_device_t *dvp,
  Noise_XX_peer_t *peer
)
{
  if (peer == NULL)
  {
  ...
  else
  {
    ...
    Lib_RandomBuffer_System_crypto_random(n8, (uint32_t)8U);
    uint64_t n = Noise_XX_bytes_to_nonce(n8);
    Noise_XX_aead_encrypt(dv.dv_sk, n, name_raw_len, name_raw, (uint32_t)32U, concat_keys, c);
    out[0U] = outb;
    outlen[0U] = (uint32_t)56U;
...
*/
Noise_XX_peer_t
*Noise_XX_deserialize_peer_secret(
  Noise_XX_device_t *dvp,
  uint8_t *peer_name,
  uint32_t inlen,
  uint8_t *enc_keys
)
{
  ...
  else
  {
    ...
    Noise_XX_error_code
    res =
      Noise_XX_aead_decrypt(dv.dv_sk,
        n0,
        name_raw_len,
        name_raw,
        (uint32_t)32U,
        concat_keys,
        c0);
    if (res == Noise_XX_CSuccess)
    {
      uint8_t *p_s = concat_keys;
      Noise_XX_device_t dv1 = dvp[0U];
      uint32_t pcounter = dv1.dv_peers_counter;
      bool b1 = pcounter == (uint32_t)4294967295U;
      Noise_XX_cell *llt = *dv1.dv_peers;
      Noise_XX_cell *lltp = llt;
      Noise_XX_cell *llt10 = lltp;
      bool b0;
      if (llt10 == NULL)
      ...
      else
      {
        Noise_XX_cell c = llt10[0U];
        Noise_XX_peer_t x = c.data[0U];
        bool b = Noise_XX_lbytes_eq((uint32_t)32U, x.p_s, p_s);
        bool b1 = b;
        b0 = !b1;
      }
      bool cond = b0;
      while (cond)
      {
        ...
        else
        {
          Noise_XX_cell c = llt10[0U];
          Noise_XX_peer_t x = c.data[0U];
          bool b0 = Noise_XX_lbytes_eq((uint32_t)32U, x.p_s, p_s);
          bool b1 = b0;
          b = !b1;
        }
        cond = b;
      ...
      {
        Noise_XX_noise_string *info1 = dv1.dv_info;
        uint8_t *sk1 = dv1.dv_sk;
        uint8_t *spriv1 = dv1.dv_spriv;
        uint8_t *spub1 = dv1.dv_spub;
        Noise_XX_sized_buffer prologue1 = dv1.dv_prologue;
        uint32_t scounter1 = dv1.dv_states_counter;
        Noise_XX_cell **peers1 = dv1.dv_peers;
        ...
        uint8_t **out_ptr = KRML_HOST_MALLOC(sizeof (uint8_t *));
        out_ptr[0U] = out_str;
        Noise_XX_noise_string *pinfo = out_ptr;
        Noise_XX_peer_t x_ = { .p_id = pcounter1, .p_info = pinfo, .p_s = rs };
        KRML_CHECK_SIZE(sizeof (Noise_XX_peer_t), (uint32_t)1U);
        Noise_XX_peer_t *xp_ = KRML_HOST_MALLOC(sizeof (Noise_XX_peer_t));
        xp_[0U] = x_;
        Noise_XX_peer_t *x = xp_;
...
*/
Noise_XX_peer_t *Noise_XX_device_lookup_peer_by_id(Noise_XX_device_t *dvp, uint32_t id)
{
  Noise_XX_device_t dv = dvp[0U];
  if (id == (uint32_t)0U)
    return NULL;
...
/*
  ...
  the peer name, etc.), then forget it.
*/
Noise_XX_peer_t *Noise_XX_device_lookup_peer_by_static(Noise_XX_device_t *dvp, uint8_t *s)
{
  Noise_XX_device_t dv = dvp[0U];
  Noise_XX_cell *llt = *dv.dv_peers;
  Noise_XX_cell *lltp = llt;
  ...
  else
  {
    Noise_XX_cell c = llt10[0U];
    Noise_XX_peer_t x = c.data[0U];
    bool b = Noise_XX_lbytes_eq((uint32_t)32U, x.p_s, s);
    bool b1 = b;
    b0 = !b1;
  }
  bool cond = b0;
  while (cond)
  {
    ...
    else
    {
      Noise_XX_cell c = llt10[0U];
      Noise_XX_peer_t x = c.data[0U];
      bool b0 = Noise_XX_lbytes_eq((uint32_t)32U, x.p_s, s);
      bool b1 = b0;
      b = !b1;
    }
    cond = b;
...
*/
void Noise_XX_device_get_info(Noise_XX_noise_string *out, Noise_XX_device_t *dvp)
{
  Noise_XX_device_t dv = dvp[0U];
...
*/
uint32_t Noise_XX_device_get_sessions_counter(Noise_XX_device_t *dvp)
{
  Noise_XX_device_t dv = dvp[0U];
...
*/
bool Noise_XX_device_sessions_counter_is_saturated(Noise_XX_device_t *dvp)
{
  Noise_XX_device_t dv = dvp[0U];
...
/*
  ...
  to give them unique identifiers.
*/
uint32_t Noise_XX_device_get_peers_counter(Noise_XX_device_t *dvp)
{
  Noise_XX_device_t dv = dvp[0U];
...
*/
bool Noise_XX_device_peers_counter_is_saturated(Noise_XX_device_t *dvp)
{
  Noise_XX_device_t dv = dvp[0U];
...
*/
void Noise_XX_device_get_static_priv(uint8_t *out, Noise_XX_device_t *dvp)
{
  Noise_XX_device_t dv = dvp[0U];
...
*/
void Noise_XX_device_get_static_pub(uint8_t *out, Noise_XX_device_t *dvp)
{
  Noise_XX_device_t dv = dvp[0U];
...
*/
uint32_t Noise_XX_peer_get_id(Noise_XX_peer_t *pp)
{
  Noise_XX_peer_t p = pp[0U];
  return p.p_id;
}
...
/*
  Copy the peer information to the user provided pointer.
*/
void Noise_XX_peer_get_info(Noise_XX_noise_string *out, Noise_XX_peer_t *pp)
{
  Noise_XX_peer_t p = pp[0U];
...
*/
void Noise_XX_peer_get_static(uint8_t *out, Noise_XX_peer_t *pp)
{
  Noise_XX_peer_t p = pp[0U];
...
typedef struct result_session_t_s
{
  ...
  union {
    Noise_XX_session_t case_Res;
    Noise_XX_error_code case_Fail;
  }
  val;
}
result_session_t;
...
*/
Noise_XX_session_t *Noise_XX_session_create_initiator(Noise_XX_device_t *dvp)
{
  uint8_t epriv[32U] = { 0U };
  uint8_t epub[32U] = { 0U };
  Lib_RandomBuffer_System_crypto_random(epriv, (uint32_t)32U);
  Noise_XX_error_code res0 = Noise_XX_dh_secret_to_public(epub, epriv);
  Noise_XX_session_t *res;
  switch (res0)
  {
    case Noise_XX_CSuccess:
      {
        ...
        if (dv.dv_states_counter == (uint32_t)4294967295U)
          res10 =
            (
              (result_session_t){
                .tag = Fail,
                .val = { .case_Fail = Noise_XX_CIncorrect_transition }
              }
            );
        else
        {
          ...
          out_ptr0[0U] = out_str;
          Noise_XX_noise_string *st_info = out_ptr0;
          KRML_CHECK_SIZE(sizeof (uint8_t *), (uint32_t)1U);
          uint8_t **out_ptr = KRML_HOST_MALLOC(sizeof (uint8_t *));
          out_ptr[0U] = NULL;
          Noise_XX_noise_string *st_pinfo = out_ptr;
          dvp[0U] =
            (
              (Noise_XX_device_t){
                .dv_info = dv.dv_info,
          ...
          pname[32U] =
            {
              ...
              (uint8_t)88U, (uint8_t)88U, (uint8_t)95U, (uint8_t)50U, (uint8_t)53U, (uint8_t)53U,
          ...
          if (st.tag == Noise_XX_IMS_Handshake)
          {
            ...
            if ((uint32_t)32U <= (uint32_t)32U)
              ...
              Noise_XX_hash(st_h, (uint32_t)32U, pname);
            memcpy(st_ck, st_h, (uint32_t)32U * sizeof (uint8_t));
            Noise_XX_mix_hash(st_h, dv.dv_prologue.size, dv.dv_prologue.buffer);
        ...
        Noise_XX_session_t *res1;
        if (res10.tag == Fail)
          res1 = NULL;
        else if (res10.tag == Res)
        {
          Noise_XX_session_t st = res10.val.case_Res;
          KRML_CHECK_SIZE(sizeof (Noise_XX_session_t), (uint32_t)1U);
          Noise_XX_session_t *ptr = KRML_HOST_MALLOC(sizeof (Noise_XX_session_t));
          ptr[0U] = st;
          res1 = ptr;
        }
        else
          res1 =
            KRML_EABORT(Noise_XX_session_t *,
              "unreachable (pattern matches are exhaustive in F*)");
        res = res1;
        break;
      }
...
/*
  ...
  May fail and return NULL in case of invalid keys, unknown peer, etc.
*/
Noise_XX_session_t *Noise_XX_session_create_responder(Noise_XX_device_t *dvp)
{
  uint8_t epriv[32U] = { 0U };
  uint8_t epub[32U] = { 0U };
  Lib_RandomBuffer_System_crypto_random(epriv, (uint32_t)32U);
...
/*
  ...
  those sessions.
*/
void Noise_XX_session_free(Noise_XX_session_t *sn)
{
  Noise_XX_session_t st = sn[0U];
  if (st.tag == Noise_XX_DS_Initiator)
  {
    Noise_XX_noise_string *pinfo = st.val.case_DS_Initiator.pinfo;
    uint8_t *spub = st.val.case_DS_Initiator.spub;
    uint8_t *spriv = st.val.case_DS_Initiator.spriv;
    Noise_XX_noise_string *info = st.val.case_DS_Initiator.info;
    Noise_XX_init_state_t state = st.val.case_DS_Initiator.state;
    if (state.tag == Noise_XX_IMS_Handshake)
    {
    ...
    else if (state.tag == Noise_XX_IMS_Transport)
    {
  ...
  else if (st.tag == Noise_XX_DS_Responder)
  {
    Noise_XX_noise_string *pinfo = st.val.case_DS_Responder.pinfo;
    uint8_t *spub = st.val.case_DS_Responder.spub;
    uint8_t *spriv = st.val.case_DS_Responder.spriv;
    Noise_XX_noise_string *info = st.val.case_DS_Responder.info;
    Noise_XX_resp_state_t state = st.val.case_DS_Responder.state;
    if (state.tag == Noise_XX_IMS_Handshake)
    {
      uint8_t *st_re = state.val.case_IMS_Handshake.re;
      uint8_t *st_rs = state.val.case_IMS_Handshake.rs;
...
typedef struct result_init_state_t_s
{
  result_session_t_tags tag;
  union {
    Noise_XX_init_state_t case_Res;
    Noise_XX_error_code case_Fail;
  }
  val;
}
...
typedef struct result_resp_state_t_s
{
  result_session_t_tags tag;
  union {
    Noise_XX_resp_state_t case_Res;
    Noise_XX_error_code case_Fail;
  }
  val;
}
...
static Noise_XX_error_code
state_handshake_write(
  uint32_t payload_len,
  uint8_t *payload,
  Noise_XX_session_t *dst_p,
  uint32_t outlen,
  uint8_t *out
)
{
  Noise_XX_session_t *dst_p1 = dst_p;
  Noise_XX_session_t *stp = dst_p1;
  Noise_XX_session_t dst = stp[0U];
  result_session_t res0;
  if (dst.tag == Noise_XX_DS_Initiator)
  {
    Noise_XX_device_t *dst_dv = dst.val.case_DS_Initiator.dv;
    Noise_XX_noise_string *dst_pinfo = dst.val.case_DS_Initiator.pinfo;
    ...
    Noise_XX_noise_string *dst_info = dst.val.case_DS_Initiator.info;
    uint32_t dst_id = dst.val.case_DS_Initiator.id;
    Noise_XX_init_state_t dst_st = dst.val.case_DS_Initiator.state;
    if (dst_st.tag == Noise_XX_IMS_Handshake)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          else
          {
            ...
            Noise_XX_encrypt_and_hash((uint32_t)32U,
              st_spub,
              tk_out,
              st_cipher,
              st_h,
              (uint64_t)1U);
            Noise_XX_error_code r0 = Noise_XX_mix_dh(st_spriv, st_re, st_cipher, st_ck, st_h);
            Noise_XX_error_code r2 = r0;
            Noise_XX_error_code r1 = r2;
            Noise_XX_error_code r;
            if (!(r1 == Noise_XX_CSuccess))
              r = r1;
            else
            {
              Noise_XX_encrypt_and_hash(payload_len,
                payload,
                payload_out,
                st_cipher,
                st_h,
            ...
            if (res0 == Noise_XX_CSuccess)
              ...
              switch (res0)
              {
                case Noise_XX_CDH_error:
                  {
                    res =
                      (
                        (result_init_state_t){
                          .tag = Fail,
                          .val = { .case_Fail = Noise_XX_CDH_error }
                        }
                      );
                    break;
                  }
        ...
        if (res1.tag == Fail)
        {
          Noise_XX_error_code e = res1.val.case_Fail;
          res0 = ((result_session_t){ .tag = Fail, .val = { .case_Fail = e } });
        }
        else if (res1.tag == Res)
        {
          Noise_XX_init_state_t st1 = res1.val.case_Res;
          Noise_XX_session_t ite;
          if (st_step == (uint32_t)2U)
          {
            Noise_XX_init_state_t st11;
            if (st1.tag == Noise_XX_IMS_Handshake)
            {
              ...
              Noise_XX_kdf(st_ck1, (uint32_t)0U, NULL, k1, k2, NULL);
              KRML_HOST_FREE(st_k);
              ...
              st11 =
                KRML_EABORT(Noise_XX_init_state_t,
                  "unreachable (pattern matches are exhaustive in F*)");
            ite =
              (
                (Noise_XX_session_t){
    ...
    else if (dst_st.tag == Noise_XX_IMS_Transport)
      res0 =
        ((result_session_t){ .tag = Fail, .val = { .case_Fail = Noise_XX_CIncorrect_transition } });
    else
      res0 = KRML_EABORT(result_session_t, "unreachable (pattern matches are exhaustive in F*)");
  }
  else if (dst.tag == Noise_XX_DS_Responder)
  {
    Noise_XX_device_t *dst_dv = dst.val.case_DS_Responder.dv;
    Noise_XX_noise_string *dst_pinfo = dst.val.case_DS_Responder.pinfo;
    ...
    Noise_XX_noise_string *dst_info = dst.val.case_DS_Responder.info;
    uint32_t dst_id = dst.val.case_DS_Responder.id;
    Noise_XX_resp_state_t dst_st = dst.val.case_DS_Responder.state;
    if (dst_st.tag == Noise_XX_IMS_Handshake)
    {
      uint8_t *st_re = dst_st.val.case_IMS_Handshake.re;
      uint8_t *st_rs = dst_st.val.case_IMS_Handshake.rs;
      ...
      else
      {
        ...
        if (!(payload_len <= (uint32_t)4294967199U && outlen == (uint32_t)96U + payload_len))
          res = ((result_resp_state_t){ .tag = Fail, .val = { .case_Fail = Noise_XX_CInput_size } });
        else
        {
          ...
          Noise_XX_error_code r11 = Noise_XX_mix_dh(st_epriv, st_re, st_cipher, st_ck, st_h);
          Noise_XX_error_code r2;
          if (r11 == Noise_XX_CSuccess)
          {
            uint8_t *out_1 = out_;
            ...
            Noise_XX_encrypt_and_hash((uint32_t)32U,
              ...
              st_h,
              (uint64_t)0U);
            Noise_XX_error_code r = Noise_XX_mix_dh(st_spriv, st_re, st_cipher, st_ck, st_h);
            Noise_XX_error_code r20 = r;
            Noise_XX_error_code r21 = r20;
            r2 = r21;
          }
          else
            r2 = r11;
        ...
        if (res2.tag == Fail)
        {
          Noise_XX_error_code e = res2.val.case_Fail;
          res0 = ((result_session_t){ .tag = Fail, .val = { .case_Fail = e } });
        }
        else if (res2.tag == Res)
        {
          Noise_XX_resp_state_t st1 = res2.val.case_Res;
          Noise_XX_session_t ite;
          if (st_step == (uint32_t)2U)
          {
            Noise_XX_resp_state_t st11;
            if (st1.tag == Noise_XX_IMS_Handshake)
            {
              uint8_t *st_re1 = st1.val.case_IMS_Handshake.re;
              uint8_t *st_rs1 = st1.val.case_IMS_Handshake.rs;
              ...
              st11 =
                KRML_EABORT(Noise_XX_resp_state_t,
                  "unreachable (pattern matches are exhaustive in F*)");
            ite =
              (
                (Noise_XX_session_t){
  ...
  if (res0.tag == Fail)
  {
    Noise_XX_error_code e = res0.val.case_Fail;
    Noise_XX_session_t dst1 = dst_p1[0U];
    if (dst1.tag == Noise_XX_DS_Initiator)
    {
      Noise_XX_device_t *dv = dst1.val.case_DS_Initiator.dv;
      Noise_XX_noise_string *pinfo = dst1.val.case_DS_Initiator.pinfo;
      ...
      Noise_XX_noise_string *info = dst1.val.case_DS_Initiator.info;
      uint32_t id = dst1.val.case_DS_Initiator.id;
      Noise_XX_init_state_t st = dst1.val.case_DS_Initiator.state;
      Noise_XX_init_state_t ite;
      if (st.tag == Noise_XX_IMS_Handshake)
      {
      ...
      else if (st.tag == Noise_XX_IMS_Transport)
      {
    ...
    else if (dst1.tag == Noise_XX_DS_Responder)
    {
      Noise_XX_device_t *dv = dst1.val.case_DS_Responder.dv;
      Noise_XX_noise_string *pinfo = dst1.val.case_DS_Responder.pinfo;
      ...
      Noise_XX_noise_string *info = dst1.val.case_DS_Responder.info;
      uint32_t id = dst1.val.case_DS_Responder.id;
      Noise_XX_resp_state_t st = dst1.val.case_DS_Responder.state;
      Noise_XX_resp_state_t ite;
      if (st.tag == Noise_XX_IMS_Handshake)
      {
        uint8_t *re = st.val.case_IMS_Handshake.re;
        uint8_t *rs = st.val.case_IMS_Handshake.rs;
  ...
  {
    Noise_XX_session_t dst1 = res0.val.case_Res;
    dst_p1[0U] = dst1;
...
{
  Noise_XX_session_t dst = dst_p[0U];
  uint32_t pid;
  if (dst.tag == Noise_XX_DS_Initiator)
  ...
  if (dst.tag == Noise_XX_DS_Initiator)
  {
    ...
    if (dst_st.tag == Noise_XX_IMS_Handshake)
    {
      ...
      {
        Noise_XX_device_t dv0 = dst_dv[0U];
        result_unit_error r0;
        if (!(payload_outlen <= (uint32_t)4294967199U && inlen == (uint32_t)96U + payload_outlen))
          r0 = ((result_unit_error){ .tag = Fail0, .v = Noise_XX_CInput_size });
        else
        {
          uint8_t *msg1 = input;
          uint8_t *msg2 = input + (uint32_t)80U;
          uint8_t *tk_input = msg1;
          Noise_XX_mix_hash(st_h, (uint32_t)32U, tk_input);
          memcpy(st_re, tk_input, (uint32_t)32U * sizeof (uint8_t));
          uint8_t *input_ = msg1 + (uint32_t)32U;
          Noise_XX_error_code r110 = Noise_XX_mix_dh(st_epriv, st_re, st_cipher, st_ck, st_h);
          Noise_XX_error_code r20;
          if (r110 == Noise_XX_CSuccess)
          {
            uint8_t *input_1 = input_;
            Noise_XX_error_code
            r =
              Noise_XX_decrypt_and_hash((uint32_t)32U,
                st_rs,
                input_1,
                st_cipher,
                st_h,
                (uint64_t)0U);
            Noise_XX_error_code r2 = r;
            r20 = r2;
          }
          else
            r20 = r110;
          Noise_XX_error_code r1 = r20;
          Noise_XX_error_code r10 = r1;
          if (r10 == Noise_XX_CSuccess)
          {
            Noise_XX_cell **peers1 = dv0.dv_peers;
            Noise_XX_cell *llt = *peers1;
            Noise_XX_cell *lltp = llt;
            Noise_XX_cell *llt10 = lltp;
            bool b0;
            if (llt10 == NULL)
            ...
            else
            {
              Noise_XX_cell c = llt10[0U];
              Noise_XX_peer_t x = c.data[0U];
              bool b = Noise_XX_lbytes_eq((uint32_t)32U, x.p_s, st_rs);
              b0 = !b;
            }
            bool cond = b0;
            while (cond)
            {
              ...
              else
              {
                Noise_XX_cell c = llt10[0U];
                Noise_XX_peer_t x = c.data[0U];
                bool b0 = Noise_XX_lbytes_eq((uint32_t)32U, x.p_s, st_rs);
                b = !b0;
              }
              cond = b;
            }
            ...
            Noise_XX_peer_t *peer_ptr = res;
            bool b1;
            if (!(peer_ptr == NULL))
            {
              Noise_XX_peer_t peer = peer_ptr[0U];
            ...
            if (r1)
            {
              ...
              Noise_XX_error_code r11 = Noise_XX_mix_dh(st_epriv, st_rs, st_cipher, st_ck, st_h);
              Noise_XX_error_code r;
              if (!(r11 == Noise_XX_CSuccess))
                r = r11;
              else
              {
                Noise_XX_error_code
                r2 =
                  Noise_XX_decrypt_and_hash(payload_outlen,
                    payload_out,
                    payload_input,
                    st_cipher,
                    st_h,
              ...
              Noise_XX_error_code r1 = r;
              Noise_XX_error_code r2 = r1;
              if (r2 == Noise_XX_CSuccess)
                r0 = ((result_unit_error){ .tag = Res0 });
              else
                r0 = ((result_unit_error){ .tag = Fail0, .v = r2 });
            }
            else
              r0 = ((result_unit_error){ .tag = Fail0, .v = Noise_XX_CRs_rejected_by_policy });
          }
          else
        ...
        {
          Noise_XX_init_state_t st1 = res2.val.case_Res;
        ...
        if (res.tag == Fail)
        {
          Noise_XX_error_code e = res.val.case_Fail;
          res0 = ((result_session_t){ .tag = Fail, .val = { .case_Fail = e } });
        }
        else if (res.tag == Res)
        {
          Noise_XX_session_t dst1 = res.val.case_Res;
          Noise_XX_session_t dst2;
          if (dst1.tag == Noise_XX_DS_Initiator)
          {
            Noise_XX_device_t *dv = dst1.val.case_DS_Initiator.dv;
            Noise_XX_noise_string *pinfo = dst1.val.case_DS_Initiator.pinfo;
  ...
  else if (dst.tag == Noise_XX_DS_Responder)
  {
    ...
    if (dst_st.tag == Noise_XX_IMS_Handshake)
    {
      ...
      else
      {
          ...
          else
          {
            ...
            r1 =
              Noise_XX_decrypt_and_hash((uint32_t)32U,
                ...
                st_h,
                (uint64_t)1U);
            Noise_XX_error_code r3 = r1;
            Noise_XX_error_code r10 = r3;
            if (r10 == Noise_XX_CSuccess)
            {
              Noise_XX_cell **peers1 = dv0.dv_peers;
              Noise_XX_cell *llt = *peers1;
        ...
        {
          Noise_XX_resp_state_t st1 = res1.val.case_Res;
          if (!(st_step == (uint32_t)1U || st_step == (uint32_t)2U))
            res =
              (
                (result_session_t){
        ...
        else if (res.tag == Res)
        {
          Noise_XX_session_t dst1 = res.val.case_Res;
          Noise_XX_session_t dst2;
          if (dst1.tag == Noise_XX_DS_Responder)
          {
            Noise_XX_device_t *dv = dst1.val.case_DS_Responder.dv;
            Noise_XX_noise_string *pinfo = dst1.val.case_DS_Responder.pinfo;
  ...
  {
    Noise_XX_error_code e = res0.val.case_Fail;
    Noise_XX_session_t dst1 = dst_p[0U];
    if (dst1.tag == Noise_XX_DS_Initiator)
    {
      Noise_XX_device_t *dv = dst1.val.case_DS_Initiator.dv;
      Noise_XX_noise_string *pinfo = dst1.val.case_DS_Initiator.pinfo;
...
)
{
  ...
  if (dst.tag == Noise_XX_DS_Initiator)
  {
    Noise_XX_device_t *dv = dst.val.case_DS_Initiator.dv;
    Noise_XX_noise_string *pinfo = dst.val.case_DS_Initiator.pinfo;
    ...
    Noise_XX_noise_string *info = dst.val.case_DS_Initiator.info;
    uint32_t id = dst.val.case_DS_Initiator.id;
    Noise_XX_init_state_t state = dst.val.case_DS_Initiator.state;
    bool ite;
    if (state.tag == Noise_XX_IMS_Handshake)
      ite = true;
    else
    ...
    if (!ite)
    {
      ...
      if (state.tag == Noise_XX_IMS_Transport)
      {
        ...
        if (!(plen <= (uint32_t)4294967279U && clen == plen + (uint32_t)16U))
          ...
          scrut =
            (
              (result_init_state_t){
                .tag = Fail,
                .val = { .case_Fail = Noise_XX_CSaturated_nonce }
              }
            );
        else
        {
          Noise_XX_aead_encrypt(send_key, send_nonce, (uint32_t)0U, NULL, plen, p, c);
          scrut =
            (
              (result_init_state_t){
                .tag = Res,
      ...
      if (scrut.tag == Fail)
      {
        Noise_XX_error_code e = scrut.val.case_Fail;
        r = ((result_session_t){ .tag = Fail, .val = { .case_Fail = e } });
      }
      else if (scrut.tag == Res)
      {
        Noise_XX_init_state_t state_ = scrut.val.case_Res;
        r =
          (
            (result_session_t){
              .tag = Res,
  ...
  {
    Noise_XX_device_t *dv = dst.val.case_DS_Responder.dv;
    Noise_XX_noise_string *pinfo = dst.val.case_DS_Responder.pinfo;
    ...
    Noise_XX_noise_string *info = dst.val.case_DS_Responder.info;
    uint32_t id = dst.val.case_DS_Responder.id;
    Noise_XX_resp_state_t state = dst.val.case_DS_Responder.state;
    bool ite;
    if (state.tag == Noise_XX_IMS_Handshake)
      ite = true;
    else
    ...
    if (!ite)
    {
      ...
      if (state.tag == Noise_XX_IMS_Transport)
      {
        ...
        if (!(plen <= (uint32_t)4294967279U && clen == plen + (uint32_t)16U))
          scrut =
            ((result_resp_state_t){ .tag = Fail, .val = { .case_Fail = Noise_XX_CInput_size } });
        else if (send_nonce >= (uint64_t)18446744073709551615U)
          scrut =
            (
              (result_resp_state_t){
      ...
      {
        Noise_XX_resp_state_t state_ = scrut.val.case_Res;
        r =
          (
            (result_session_t){
              .tag = Res,
  ...
  {
    Noise_XX_session_t dst_ = r.val.case_Res;
    dst_p[0U] = dst_;
    return Noise_XX_CSuccess;
  }
  else
    ...
    if (!ite)
    {
      ...
      if (state.tag == Noise_XX_IMS_Transport)
      {
        ...
        if (!(plen <= (uint32_t)4294967279U && clen == plen + (uint32_t)16U))
          ...
          switch (Noise_XX_aead_decrypt(receive_key, receive_nonce, (uint32_t)0U, NULL, plen, p, c))
          {
            case Noise_XX_CDecrypt_error:
              {
                scrut =
                  (
                    (result_init_state_t){
                      .tag = Fail,
                      .val = { .case_Fail = Noise_XX_CDecrypt_error }
                    }
                  );
                break;
              }
...
*/
Noise_XX_rcode
Noise_XX_session_write(
  Noise_XX_encap_message_t *payload,
  Noise_XX_session_t *sn_p,
  uint32_t *out_len,
  uint8_t **out
)
{
  Noise_XX_session_t *sn_p1 = sn_p;
  Noise_XX_session_t *snp = sn_p1;
  Noise_XX_session_t sn = snp[0U];
  if (sn.tag == Noise_XX_DS_Initiator)
  {
    Noise_XX_init_state_t sn_state = sn.val.case_DS_Initiator.state;
    if (sn_state.tag == Noise_XX_IMS_Transport)
    {
      bool recv_tpt_msg = sn_state.val.case_IMS_Transport.recv_transport_message;
      Noise_XX_encap_message_t encap_payload = payload[0U];
      bool next_length_ok;
      ...
      if (next_length_ok)
      {
        ...
        else
        {
          ...
          if (recv_tpt_msg)
            ...
            clevel = (uint8_t)5U;
          if (encap_payload.em_ac_level.tag == Noise_XX_Conf_level)
          {
        ...
        if (sec_ok)
        {
          ...
          {
            out[0U] = out1;
            return ((Noise_XX_rcode){ .tag = Noise_XX_Success });
          }
          else
          {
            Noise_XX_error_code e = res;
            KRML_HOST_FREE(out1);
            out_len[0U] = (uint32_t)0U;
            out[0U] = NULL;
            return ((Noise_XX_rcode){ .tag = Noise_XX_Error, .val = { .case_Error = e } });
          }
        }
        else
        {
          ...
          return
            (
              (Noise_XX_rcode){
                .tag = Noise_XX_Error,
                .val = { .case_Error = Noise_XX_CSecurity_level }
              }
            );
        }
      }
      else
      {
        ...
        out[0U] = NULL;
        return
          ((Noise_XX_rcode){ .tag = Noise_XX_Error, .val = { .case_Error = Noise_XX_CInput_size } });
      }
    }
    else if (sn_state.tag == Noise_XX_IMS_Handshake)
    {
      ...
      if (st_step >= (uint32_t)4U)
      {
        ...
        return
          (
            (Noise_XX_rcode){
              .tag = Noise_XX_Stuck,
              .val = { .case_Stuck = Noise_XX_CIncorrect_transition }
            }
          );
      }
      else
        ...
        if (next_length_ok)
        {
          ...
          if (sec_ok)
          {
            ...
            else
            {
              ...
              out_len[0U] = (uint32_t)0U;
              out[0U] = NULL;
              return ((Noise_XX_rcode){ .tag = Noise_XX_Stuck, .val = { .case_Stuck = e } });
            }
          }
          else
          {
        ...
        else
        {
          ...
          return
            (
              (Noise_XX_rcode){
                .tag = Noise_XX_Error,
                .val = { .case_Error = Noise_XX_CInput_size }
              }
            );
        }
      }
  ...
  else if (sn.tag == Noise_XX_DS_Responder)
  {
    Noise_XX_resp_state_t sn_state = sn.val.case_DS_Responder.state;
    if (sn_state.tag == Noise_XX_IMS_Transport)
    {
      Noise_XX_encap_message_t encap_payload = payload[0U];
      bool next_length_ok;
...
*/
Noise_XX_rcode
Noise_XX_session_read(
  Noise_XX_encap_message_t **payload_out,
  Noise_XX_session_t *sn_p,
  uint32_t inlen,
  uint8_t *input
)
    ...
    if (sn_state.tag == Noise_XX_IMS_Transport)
    {
      ...
      if (scrut.tag == Some)
      {
        ...
        if (outlen > (uint32_t)0U)
        {
          ...
          out = NULL;
        Noise_XX_error_code res = state_transport_read(outlen, out, inlen, input, sn_p1);
        if (res == Noise_XX_CSuccess)
        {
          KRML_CHECK_SIZE(sizeof (Noise_XX_encap_message_t), (uint32_t)1U);
          Noise_XX_encap_message_t *em_ptr = KRML_HOST_MALLOC(sizeof (Noise_XX_encap_message_t));
          em_ptr[0U]
          =
            (
              (Noise_XX_encap_message_t){
                .em_ac_level = {
                  .tag = Noise_XX_Auth_level,
          ...
          Noise_XX_encap_message_t *emp = em_ptr;
          payload_out[0U] = emp;
          return ((Noise_XX_rcode){ .tag = Noise_XX_Success });
        }
        else
      ...
      else if (st_step % (uint32_t)2U == (uint32_t)1U && st_step < (uint32_t)3U)
      {
        ...
        if (scrut.tag == Some)
        {
          ...
          if (outlen > (uint32_t)0U)
          {
            ...
            out = NULL;
          Noise_XX_error_code res = state_handshake_read(outlen, out, sn_p1, inlen, input);
          if (res == Noise_XX_CSuccess)
          {
            KRML_CHECK_SIZE(sizeof (Noise_XX_encap_message_t), (uint32_t)1U);
            Noise_XX_encap_message_t *em_ptr = KRML_HOST_MALLOC(sizeof (Noise_XX_encap_message_t));
      ...
      else
      {
        ...
        return
          (
            (Noise_XX_rcode){
              .tag = Noise_XX_Error,
              .val = { .case_Error = Noise_XX_CIncorrect_transition }
            }
          );
      }
    }
...
*/
bool
Noise_XX_session_compute_next_message_len(
  uint32_t *out,
  Noise_XX_session_t *sn,
  uint32_t payload_len
)
{
  Noise_XX_session_t dst = sn[0U];
  if (dst.tag == Noise_XX_DS_Initiator)
  {
    Noise_XX_init_state_t st = dst.val.case_DS_Initiator.state;
    if (st.tag == Noise_XX_IMS_Handshake)
    {
  ...
  {
    Noise_XX_resp_state_t st = dst.val.case_DS_Responder.state;
    if (st.tag == Noise_XX_IMS_Handshake)
    {
      uint32_t step = st.val.case_IMS_Handshake.step;
      option__uint32_t scrut;
...
*/
Noise_XX_status Noise_XX_session_get_status(Noise_XX_session_t *sn)
{
  Noise_XX_session_t dst = sn[0U];
  if (dst.tag == Noise_XX_DS_Initiator)
  {
    ...
    if (st.tag == Noise_XX_IMS_Handshake)
    {
      ...
      if (step % (uint32_t)2U == (uint32_t)0U)
        return Noise_XX_Handshake_write;
      else
        return Noise_XX_Handshake_read;
    }
    else if (st.tag == Noise_XX_IMS_Transport)
      return Noise_XX_Transport;
    else
    {
      KRML_HOST_EPRINTF("KaRaMeL abort at %s:%d\n%s\n",
        __FILE__,
...
/*
  ...
  transport phase.
*/
void Noise_XX_session_get_hash(uint8_t *out, Noise_XX_session_t *sn)
{
  Noise_XX_session_t dst = sn[0U];
  uint8_t *h;
  if (dst.tag == Noise_XX_DS_Initiator)
...
*/
uint32_t Noise_XX_session_get_id(Noise_XX_session_t *sn)
{
  Noise_XX_session_t st = sn[0U];
  if (st.tag == Noise_XX_DS_Initiator)
...
*/
void Noise_XX_session_get_info(Noise_XX_noise_string *out, Noise_XX_session_t *sn)
{
  Noise_XX_session_t st = sn[0U];
  if (st.tag == Noise_XX_DS_Initiator)
  {
...
*/
uint32_t Noise_XX_session_get_peer_id(Noise_XX_session_t *sn)
{
  Noise_XX_session_t st = sn[0U];
  if (st.tag == Noise_XX_DS_Initiator)
...
*/
bool Noise_XX_session_get_peer_info(Noise_XX_noise_string *out, Noise_XX_session_t *sn)
{
  Noise_XX_session_t st = sn[0U];
  if (st.tag == Noise_XX_DS_Initiator)
  {
...
/*
  ...
  handshake.
*/
bool Noise_XX_session_reached_max_security(Noise_XX_session_t *snp)
{
  Noise_XX_session_t sn = snp[0U];
  if (sn.tag == Noise_XX_DS_Initiator)
  {
...
*/
Noise_XX_session_t
*Noise_XX__session_create_initiator_with_ephemeral(
  Noise_XX_device_t *dvp,
  uint8_t *epriv,
  uint8_t *epub
)
{
  ...
  {
    Noise_XX_session_t st = res.val.case_Res;
    KRML_CHECK_SIZE(sizeof (Noise_XX_session_t), (uint32_t)1U);
    Noise_XX_session_t *ptr = KRML_HOST_MALLOC(sizeof (Noise_XX_session_t));
    ptr[0U] = st;
    return ptr;
...
*/
Noise_XX_session_t
*Noise_XX__session_create_responder_with_ephemeral(
  Noise_XX_device_t *dvp,
  uint8_t *epriv,
  uint8_t *epub
)
