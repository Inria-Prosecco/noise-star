...
)
{
  ...
  Noise_XX_hmac(secret, (uint32_t)64U, hash1, keylen, key);
  if (!(dst1 == NULL))
  {
    output[0U] = (uint8_t)1U;
    Noise_XX_hmac(output_hash, (uint32_t)64U, secret, (uint32_t)1U, output1);
    memcpy(dst1, output_hash, (uint32_t)64U * sizeof (uint8_t));
    if (!(dst2 == NULL))
    {
      output[64U] = (uint8_t)2U;
      Noise_XX_hmac(output_hash, (uint32_t)64U, secret, (uint32_t)65U, output);
...
{
  Noise_XX_aead_encrypt(st_cs_k, nonce, (uint32_t)64U, st_h, msg_len, msg, cipher);
...
{
  Noise_XX_error_code
  r1 = Noise_XX_aead_decrypt(st_cs_k, nonce, (uint32_t)64U, st_h, msg_len, msg, cipher);
...
Noise_XX_mix_dh(uint8_t *sec, uint8_t *pub, uint8_t *cipher_key, uint8_t *ck, uint8_t *hash1)
{
  ...
  {
    uint8_t temp_k[64U] = { 0U };
    Noise_XX_kdf(ck, (uint32_t)32U, dh_key, ck, temp_k, NULL);
